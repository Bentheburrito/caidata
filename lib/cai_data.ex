defmodule CAIData do
  ...
  def get_session(character_id) do
    CAIData.Repo.one(
      from(s in CAIData.CharacterSession,
        select: s,
        where: ilike(s.character_id, ^character_id),
        limit: 1
      )
    )
  end
  ...
  def get_session_by_name(character_name) do
    CAIData.Repo.one(
      from(s in CAIData.CharacterSession,
        select: s,
        where: ilike(s.name, ^character_name),
        limit: 1
      )
    )
  end
  ...
  def get_all_sessions(character_id) do
    CAIData.Repo.all(
      from(s in CAIData.CharacterSession, select: s, where: ilike(s.character_id, ^character_id))
    )
  end
end
