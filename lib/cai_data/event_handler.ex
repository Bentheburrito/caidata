defmodule CAIData.EventHandler do
    ...
    case SessionHandler.get(character_id) do
      {:ok, %CharacterSession{} = session} ->
        CharacterSession.changeset(session, %{
          ...
          deaths_ivi: session.deaths_ivi + ((is_ivi_kill == true && 1) || 0)
        })
        |> SessionHandler.update()
  ...
  def handle_event(
    ...
    with {:ok, vehicle} <- Map.fetch(CAIData.vehicle_info(), attacker_vehicle_id) do
      ...
      case SessionHandler.get(attacker_id) do
        {:ok, %CharacterSession{vehicles_destroyed: vehicles_destroyed} = session}
        when attacker_id != character_id ->
          CharacterSession.changeset(session, %{
  ...
  def handle_event(
        {"MetagameEvent",
         %{"metagame_event_id" => metagame_event_id, "metagame_event_state" => "135"}}
      ) do
    ...
    event = {Map.get(CAIData.event_info(), event_id), user_ids}
    PubSub.broadcast(:ps2_events, "metagame", event)
  end

  def handle_event({"ContinentLock", %{"world_id" => world_id, "zone_id" => zone_id}}) do
    event = {Map.get(CAIData.world_info(), world_id), Map.get(CAIData.zone_info(), zone_id)}
