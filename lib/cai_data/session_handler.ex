defmodule CAIData.SessionHandler do
  ...
  def handle_call({:get_by, field, value}, _from, {session_map, pending_ids})
  ...
  def handle_info(
        {:fetch_new_sessions, :end, {char_list, remaining_pending_ids}},
        {session_map, pending_ids}
      ) do
  ...
  def handle_info({:archive_old_sessions, :start}, state) do
    Task.start(fn ->
      unarchived_sessions =
        CAIData.Repo.all(
          from(s in CAIData.CharacterSession,
            select: s,
  ...
  def handle_info(
        {:archive_old_sessions, :end, {char_list, unarchived_sessions}},
        {session_map, pending_ids}
      ) do
    Enum.each(unarchived_sessions, fn %CharacterSession{} = session ->
      with character when not is_nil(character) <-
             Enum.find(char_list, &(&1["character_id"] == session.character_id)) do
        {end_fire_count, end_hit_count} =
          count_weapon_stats(Map.get(character, "weapon_shot_stats", %{}))

        if session.shots_fired < end_fire_count or
             DateTime.utc_now() |> DateTime.to_unix() >
               session.logout_timestamp + @max_time_unarchived do
          params = %{
  ...
  defp fetch_char_list(character_ids) do
    ...
    case PS2.API.query(char_query) do
      ...
      # Fatal error in query, so just discard the character_ids that cause it.
      e ->
  ...
  defp char_list_to_sessions(char_list, session_map) do
    Enum.reduce(char_list, session_map, fn
      ...
      sessions
      when is_map_key(sessions, character_id) ->
  ...
  defp schedule_work(:new_sessions),
    do: Process.send_after(self(), {:fetch_new_sessions, :start}, 15 * 1000)

  # (15 seconds for testing) Every 60 seconds.
  defp schedule_work(:archive_sessions),
    do: Process.send_after(self(), {:archive_old_sessions, :start}, 15 * 1000)
