defmodule CAIData.WorldState do
  ...
  def add_population(world_id, faction_id, pop),
    do: GenServer.cast(__MODULE__, {:add_pop, world_id, faction_id, pop})

  def get_population(world_id, faction_id),
    do: GenServer.call(__MODULE__, {:get_pop, world_id, faction_id})

  def get_population(world_id), do: GenServer.call(__MODULE__, {:get_pop, world_id})

  def put_bastion(world_id, zone_id, faction_id, outfit_alias),
    do: GenServer.cast(__MODULE__, {:put_bastion, world_id, zone_id, faction_id, outfit_alias})

  def damage_bastion(world_id, zone_id, npc_id) when is_binary(npc_id),
    do:
      GenServer.cast(__MODULE__, {:damage_bastion, world_id, zone_id, String.to_integer(npc_id)})

  def damage_bastion(world_id, zone_id, npc_id),
    do: GenServer.cast(__MODULE__, {:damage_bastion, world_id, zone_id, npc_id})

  def remove_bastion(world_id, zone_id, faction_id),
    do: GenServer.cast(__MODULE__, {:remove_bastion, world_id, zone_id, faction_id})
  ...
  def handle_cast(
        {:put_bastion, world_id, zone_id, faction_id, outfit_alias},
        {population_state, bastion_state}
      ) do
    {:noreply,
     {population_state,
      Map.put(bastion_state, {world_id, zone_id, faction_id}, %{
        faction_id: faction_id,
        health: 8,
        outfit_alias: outfit_alias,
        comp_set: []
      })}}
  end
  ...
  def handle_cast({:damage_bastion, world_id, zone_id, npc_id}, {population_state, bastion_state}) do
    ...
    case get_component_bastion(relevant_bastions, npc_id) do
      %{faction_id: faction_id} ->
        new_bastion_state =
          Utilities.Map.update_existing(
            bastion_state,
            {world_id, zone_id, faction_id},
            &%{&1 | health: &1.health - 1, comp_set: [npc_id | &1.comp_set]}
          )

        {:noreply, {population_state, new_bastion_state}}

      _ ->
        {:noreply, {population_state, bastion_state}}
    end
  end

  def handle_cast(
        {:remove_bastion, world_id, zone_id, faction_id},
        {population_state, bastion_state}
      ) do
  ...
  def handle_call(
        {:get_pop, world_id, faction_id},
        _from,
        {population_state, _bastion_state} = state
      ) do
